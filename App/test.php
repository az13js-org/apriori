...
$data = new Apriori\Apriori(0.005, 0.3, $datas);
...
file_put_contents('test.csv', "FROM,TO,SUPPORT,CONFIDENCE,BELIEVE" . PHP_EOL);
foreach ($data->getAssociationRule()->getAssociationPairs() as $pair) {
	...
	file_put_contents('test.csv', "\"$from\",\"$to\",$support,$confidence,$believe" . PHP_EOL, FILE_APPEND);
}
